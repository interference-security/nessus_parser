...
my $help_msg = '
NAME
    parse_nessus_xml.v22.pl -- parse nessus v2 XML files into an XLSX file
    
SYNOPSIS
    perl parse_nessus_xml.v22.pl [-vVhH] [-f file] [-d directory] [-r recast_file optional ]

DESCRIPTION
    Nessus Parser v0.22 - This is a program to parse a series of Nessus XMLv2
    files into a XLSX file.  The data from the XML file is placed into a series
    of tabs to for easier review and reporting.  New features with this edition
    are better reporting of policy plugin families, user account reporting,
    summary graphs, and a home page with summary data.  For more information
    ...
    EXAMPLES
        The command:
                perl /path/to/script/parse_nessus_xml.v22.pl -v
            
            This command will print this help message.
        
        The command:
                perl /path/to/script/parse_nessus_xml.v22.pl -h
            
            This command will print this help message.
        
        The command:
                perl /path/to/script/parse_nessus_xml.v22.pl -d /foo/bar
            
            This command will seearch the direcoty specified by the "-d" option
            for Nessus XML v2 files and parse the files found.
        
        The command:
                perl /path/to/script/parse_nessus_xml.v22.pl -f /foo/bar/scan1.nessus
                -----  or -----
                perl /path/to/script/parse_nessus_xml.v22.pl -f /foo/bar/scan1.nessus.xml
            
            This command will seearch the direcoty specified by the "-d" option
            for Nessus XML v2 files and parse the files found.
            
        The command:
                perl /path/to/script/parse_nessus_xml.v22.pl -f /foo/bar/scan1.nessus -r /path/to/script/recast.txt
...
sub store_vuln{
    ...
    foreach my $vuln (@vuln_array){
        if (!$vuln->{compliance}) {$vuln_totals{$vuln->{-severity}}->{$vuln->{-pluginID}}++;}
